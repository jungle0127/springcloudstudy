...
public class OrderService {
    ...
    private final String CREATE_ORDER_SQL = "INSERT INTO t_order(`customer_id`,`amount`,`order_desc`) VALUES (?,?,?)";
    private final String UPDATE_STOREAGE_SQL = "UPDATE t_storeage SET inventory = inventory - ?, update_time = now() WHERE id = ?";

    @Transactional
    public Boolean createOrder(Order order){
        try{
            int affectedOrderRows =  this.orderJdbcTemplate.update(CREATE_ORDER_SQL,order.getCustomerId(), order.getAmmount(),order.getOrderDesc());
